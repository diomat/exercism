...
Elm.Bob.make = function (_elm) {
   ...
   var isBetween = F3(function (low,high,$char) {
      var code = $Char.toCode($char);
      return _U.cmp(code,$Char.toCode(low)) > -1 && _U.cmp(code,$Char.toCode(high)) < 1;
   });
   var isCharacter = function ($char) {    return A3(isBetween,_U.chr("a"),_U.chr("z"),$char) || A3(isBetween,_U.chr("A"),_U.chr("Z"),$char);};
   var clearText = function (text) {    return A2($String.filter,isCharacter,text);};
   var isShouting = function (text) {
      var text$ = clearText(text);
      var bla = $Debug.log(text$);
      return A2($String.all,$Char.isUpper,text$) ? A2($Debug.log,text$,true) : false;
   };
   var isQuestion = function (text) {    return A2($String.endsWith,"?",text) ? true : false;};
   var hey = function (text) {
      return $String.isEmpty($String.trim(text)) ? "Fine. Be that way!" : isShouting(text) ? "Whoa, chill out!" : isQuestion(text) ? "Sure." : "Whatever.";
   };
   var main = $Graphics$Element.show($Basics.toString(isShouting("1, 2, 3")));
   return _elm.Bob.values = {_op: _op
                            ,main: main
                            ,hey: hey
